#!/usr/bin/python
import numpy
from SheetShelfSolver import SheetShelfSolver
...
if(options.inFile == "none" or options.inFile == "zero"):
  ...
  if(options.inFile == "none"):
    ...
    xShelf = xgGuess + (solver.xc-xgGuess)*solver.cheb.x
    const = (solver.delta*solver.a/(8.*solver.epsilon))**solver.n
    uShelf = (const*(xShelf**(solver.n+1)-xgGuess**(solver.n+1)) + uxg**(solver.n+1))**(1./(solver.n + 1))
    HGuess[Nx:2*Nx] = solver.a*xShelf/uShelf
  
  if(options.inFile == "zero"):
    print "initializing with zero."
    HGuess = 1e-4*numpy.ones((2*solver.Nx))
    xgGuess = 0.7
...
#    solver.xg += solver.dt*prev_dxg_dt
  ...
  for inner in range(maxInnerSteps):
    ...
    uEff = solver.u[0:Nx] - solver.cheb.x*dxg_dt
    cflSheet = numpy.amax(numpy.abs(uEff/(solver.xg*deltaX)))*solver.dt
    uEff = solver.u[Nx:2*Nx] - (1.-solver.cheb.x)*dxg_dt
    cflShelf = numpy.amax(numpy.abs(uEff/((solver.xc-solver.xg)*deltaX)))*solver.dt
