...
commonArgs = ["python", "../../code/SheetShelfSolver.py"] \
  + commonArgs \
...
if (restartFile == "none"):
  ...
  args = commonArgs + ["--outFile=%s.pyda"%initFile, "--eps_s=1e-8", 
    "--tolerance=1e-10"]
...
if (restartFile != "%s.pyda"%strictFile) and ( restartFile != "%s.pyda"%finalFile): 
  ...
  args = commonArgs + ["--outFile=%s.pyda"%looseFile, "--eps_s=1e-8", 
    "--tolerance=1e-10", "--inFile=%s"%restartFile]
  status = subprocess.call(args, stdout=logFile, stderr=errFile)
  logFile.close()
  errFile.close()
  if status != 0:
