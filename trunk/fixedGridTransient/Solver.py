...
class Solver:
  def __init__(self, options):
    ...
    self.writeToSeparateFile = options.writeToSeparateFile
    self.initFromCheb = options.initFromCheb
  ...
  def readResults(self, fileName):
    ...
    # read from Chebyshev file results and interpolate
    if self.initFromCheb:
        try:
  ...
  def writeResults(self):
    if self.writeToSeparateFile:
        fileName = "%s/%s.%04i"%(self.folder,self.outFile,self.outputFileIndex)
    else:
...
parser.add_option("--C", type="float", default=3.160e6, dest="C")
...
parser.add_option("--inFile", type="string", default="C_7.624e6_adv_final.pyda", dest="inFile")
...
parser.add_option("--initFromCheb", action="store_true", default=False, dest="initFromCheb")
...
parser.add_option("--toleranceInner", type="float", default=5e-2, dest="toleranceInner")
...
parser.add_option("--xgInit", type="float", default=0.8, dest="xgInit")
parser.add_option("--goalCFL", type="float", default=0.5, dest="goalCFL")
